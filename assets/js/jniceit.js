   ...
   //    	}
	  ...
	  //   });
	    ...
	    $(ctrl).click(function () {
		  	...
		  	targetInputID = 'b#fmChbx-' + $(ctrl).attr('id');

	    	// console.log(' c ');
	    	// console.log( $(targetInputID).hasClass('checked') );

				if( $(targetInputID).hasClass('checked') ) {
					$(targetInputID).removeClass('checked').attr('checked', false);
					$(ctrl).prop("checked", false);

				} else {
					$(targetInputID).addClass('checked').attr('checked', true);
	...
	$.fn.fnReplaceRadioButtons = function() {
      ...
      $(ctrl).after('<a rel="' + $(ctrl).attr('name') + '" class="fmRadio' + ($(ctrl).attr('checked') ? ' checked' : '') + '" id="' + id + '">&nbsp;</a>');
			...
			$(ctrl).bind('click', function () {
				...
				$('input[name="' + $(nCtrl).attr('rel') + '"]').prop('checked', false);
				$(nCtrl).addClass('checked');
